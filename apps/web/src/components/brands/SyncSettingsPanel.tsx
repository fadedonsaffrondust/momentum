...
export function SyncSettingsPanel({ brand, stakeholders, onClose }: Props) {
  ...
  return createPortal(
    <>
      ...
      <div className="fixed right-0 top-0 bottom-0 z-50 w-96 bg-m-bg border-l border-m-border shadow-2xl flex flex-col animate-slideLeft">
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-6">
          {/* Stakeholder emails */}
          <div>
            <label className="text-[10px] uppercase tracking-widest text-m-fg-muted font-semibold">
              Stakeholder Emails for Matching
            </label>
            <p className="text-xs text-m-fg-dim mt-1 mb-2">
            ...
            {stakeholderEmails.length > 0 ? (
              <ul className="space-y-1">
                {stakeholderEmails.map((s) => (
                  <li
                    key={s.email}
                    className="flex items-center gap-2 px-2 py-1 rounded bg-m-surface-50 text-xs"
                  >
                    <span className="text-m-fg-secondary">{s.name}</span>
                    <span className="text-m-fg-dim">{s.email}</span>
                  </li>
                ))}
              </ul>
            ) : (
              <p className="text-xs text-m-fg-dim italic">No stakeholder emails configured.</p>
            )}
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-m-bg border border-m-border rounded-lg px-3 py-2 text-sm text-m-fg-strong focus:outline-none focus:border-accent placeholder:text-m-fg-dim"
            />
          </div>
          ...
          <div>
            ...
            <select
              ...
              className="w-full bg-m-bg border border-m-border rounded-lg px-3 py-2 text-sm text-m-fg-strong focus:outline-none focus:border-accent"
            >
        ...
        <div className="px-5 py-4 border-t border-m-border flex gap-2">
          <button
            onClick={onClose}
            className="flex-1 px-3 py-2 rounded-md border border-m-border text-sm text-m-fg-tertiary hover:bg-m-surface-hover transition"
          >
            Cancel
          </button>
          <button
            ...
            className="flex-1 px-3 py-2 rounded-md bg-accent hover:bg-accent-hover text-sm transition disabled:opacity-50"
          >
